"""Tests for Clicker.py that deliberately exercise failure paths as well as happy paths."""
...
MODULE_PATH = Path(__file__).with_name("Clicker.py")
spec = importlib.util.spec_from_file_location("clicker", MODULE_PATH)
clicker = importlib.util.module_from_spec(spec)
spec.loader.exec_module(clicker)


def test_press_key_normalizes_names_and_releases_after_hold(monkeypatch):
    calls = []
    monkeypatch.setattr(clicker, "_send_key_event",
                        lambda vk, extended, key_up: calls.append((vk, extended, key_up)))
    monkeypatch.setattr(clicker.time, "sleep", lambda seconds: None)

    clicker.press_key("  RIGHT CTRL ")
...
def test_press_key_attempts_release_when_hold_sleep_fails(monkeypatch):
    ...
    monkeypatch.setattr(clicker.time, "sleep", fail_sleep)

    with pytest.raises(RuntimeError, match="interrupted"):
        clicker.press_key("scroll lock")
...
def test_key_options_include_requested_modifiers_and_lock_keys():
    ...
    assert requested <= {option.lower() for option in clicker.KEY_OPTIONS}
    assert clicker.DEFAULT_KEY in clicker.KEY_OPTIONS


def test_send_key_event_is_explicitly_unsupported_off_windows(monkeypatch):
    monkeypatch.setattr(clicker, "user32", None)

    with pytest.raises(OSError, match="only available on Windows"):
        clicker._send_key_event(0x91, False, False)
...
def test_dual_slider_rejects_invalid_configuration(kwargs):
    with pytest.raises(ValueError):
        clicker.DualSlider(None, **kwargs)


def test_dual_slider_coordinate_conversion_clamps_input():
    slider = clicker.DualSlider.__new__(clicker.DualSlider)
...
def app_without_gui(key="scroll lock"):
    app = clicker.KeyPresserApp.__new__(clicker.KeyPresserApp)
    app.running = False
    app.press_count = 0
...
def test_start_rejects_unknown_key_before_creating_worker(monkeypatch):
    ...
    created = []
    monkeypatch.setattr(clicker.threading, "Thread",
...
def test_worker_stops_cleanly_when_event_is_set_before_press(monkeypatch):
    ...
    event.set()
    pressed = []
    monkeypatch.setattr(clicker, "press_key", lambda key: pressed.append(key))
...
def test_worker_reports_unexpected_exception_and_does_not_claim_running(monkeypatch):
    ...
    monkeypatch.setattr(clicker, "press_key", fail)
